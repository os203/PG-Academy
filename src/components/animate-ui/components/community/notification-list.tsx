...
function NotificationList({ notifications = defaultNotifications, onViewAll }: NotificationListProps) {
  return (
    <motion.div
      className="bg-neutral-200 dark:bg-neutral-900 p-3 rounded-3xl space-y-3 shadow-md w-full max-w-sm"
      initial="collapsed"
      whileHover="expanded"
    >
      <div>
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="grid">
          ...
          <motion.button
            onClick={() => onViewAll ? onViewAll() : alert('View all notifications clicked!')}
            className="text-sm font-medium text-neutral-600 dark:text-neutral-300 flex items-center gap-1 cursor-pointer select-none row-start-1 col-start-1 hover:text-brand-primary dark:hover:text-brand-primary transition-colors focus:outline-none"
