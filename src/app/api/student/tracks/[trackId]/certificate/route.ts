...
export async function POST(
  ...
  try {
    ...
    if (!enrollment) {
      return new NextResponse("Not enrolled in this track", { status: 403 });
    }
    ...
    // 3. Fetch track structure to evaluate completion
    ...
    if (!isComplete) {
      return new NextResponse("Track not fully completed yet", { status: 400 });
    }
    ...
    if (user) {
      try {
        await sendCertificateEmail(user.email, user.name, track.title, uniqueCode);
      } catch (err) {
